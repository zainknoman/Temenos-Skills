...
## Application Field Prefix Rule

T24 file layouts define a prefix (`PREFIX[ECB.]`). In jBC component code, field aliases always include this prefix converted to PascalCase — this prevents alias collisions when multiple applications are used in one component.

**Derive from file layout header:**
```
* File Layout for EB.CONTRACT.BALANCES
*      PREFIX[ECB.]      SUFFIX[]
           EQU ECB.CURRENCY TO 1, EbContractBalances_Currency TO 1,
           EQU ECB.TYPE.SYSDATE TO 2, EbContractBalances_TypeSysdate TO 2,
```
→ prefix `ECB.` → PascalCase prefix `Ecb`

**Field alias = Ecb prefix + field-name-in-PascalCase:**
| Prefixed field name | Component field alias |
|--------------------|----------------------|
| `ECB.CURRENCY` | `EcbCurrency` |
| `ECB.TYPE.SYSDATE` | `EcbTypeSysdate` |
| `ECB.OPEN.BALANCE` | `EcbOpenBalance` |

**In `.component` table declaration — use prefixed aliases:**
```
public table EbContractBalances {
    t24: EB.CONTRACT.BALANCES
    fields: {
        EcbCurrency    = 1
        EcbTypeSysdate = 2
    }
...
**In `.b` file field access — alias includes prefix:**
```jBC
    ecbRecord = BF.ConBalanceUpdates.EbContractBalances.Read(accountId, readError)

    typeSysdate = ecbRecord<BF.ConBalanceUpdates.EbContractBalances.EcbTypeSysdate>
    openBalance = ecbRecord<BF.ConBalanceUpdates.EbContractBalances.EcbOpenBalance>
```

**Never use unprefixed aliases:**
```
fields: {
    Currency    = 1    ;* WRONG — missing Ecb prefix
    TypeSysdate = 2    ;* WRONG — missing Ecb prefix
}
```
...
Announce:
> "Using temenos-jbc skill. Mode: DEVELOP. Analysing requirement..."
