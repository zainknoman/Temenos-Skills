...
Type `/temenos-l3` at the start of your request, or simply describe what you want to build.  
...
```
/temenos-l3 write a validateRecord hook for CUSTOMER that checks NATIONALITY is mandatory for individual customers
```

```
/temenos-l3 create a ServiceLifecycle batch that selects all PENDING records from MY.TABLE and processes them
```

```
/temenos-l3 write a postCoreTableUpdate for an AA LENDING arrangement that writes to a custom table after authorisation
```
...
Ask Claude:
```
/temenos-l3 generate the skeleton for a RecordLifecycle validateRecord hook
...
```
/temenos-l3 in the validateRecord for CUSTOMER:
...
```
/temenos-l3 in defaultFieldValues read the LC.TYPES record for the current
...
```
/temenos-l3 write a defaultFieldValues ActivityLifecycle hook that reads
...
```
/temenos-l3 review this code for CSD coding standards violations
```
...
```
/temenos-l3 what EB.API record do I need for this hook?
```
...
```
/temenos-l3 write a complete RecordLifecycle validateRecord hook for
...
```
/temenos-l3 write a ServiceLifecycle batch that:
...
```
/temenos-l3 how do I calculate the number of working days between two dates?
```

```
/temenos-l3 how do I read the available balance of an account and compare
it against a transaction amount using BigDecimal?
```

#### Understand a method signature
```
/temenos-l3 what is the correct method signature for postCoreTableUpdate
in ActivityLifecycle and which TransactionData import do I use?
```

#### Fix a CSD violation
```
/temenos-l3 I'm using double for an amount calculation — how do I convert
...
```
C:\Users\Lenovo\.claude\skills\temenos-l3\
