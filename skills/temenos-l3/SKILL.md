...
### 12. Application field prefix — Java getter/setter naming

Java getter/setter names on generated record classes do **NOT** include the application prefix. The prefix (`ECB.`, `AC.`, etc.) is stripped — only the field-name part remains, in PascalCase.
...
```
→ Java getter: `getTypeSysdate()` — prefix `ECB` is dropped, only `TypeSysdate` part used.

**Always look up the exact getter in** `com\temenos\t24\api\records\<ApplicationName>\<ApplicationNameRecord>.java` — the generated class is authoritative.
...
| Layer | Prefix used? | Example |
|-------|-------------|---------|
| Infobasic | Yes | `ECB.TYPE.SYSDATE` |
| jBC-componentise | Yes (PascalCase) | `EcbTypeSysdate` |
| Java getter | **No** | `getTypeSysdate()` |
