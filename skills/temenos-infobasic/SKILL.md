---
name: temenos-temenos-infobasic
description: >
...
Every T24 application file layout defines a prefix (`PREFIX[XXX.]`). Always use the full prefixed field name in Infobasic — never the bare field name alone.

**Derive prefix from file layout header:**
```
* File Layout for EB.CONTRACT.BALANCES Created 12 APR 19 at 11:34AM
*      PREFIX[ECB.]      SUFFIX[]
...
**Always use the prefixed field name:**
```
EQU ECB.CURRENCY    TO 1
EQU ECB.TYPE.SYSDATE TO 2

lvCurrency   = lvRecord<ECB.CURRENCY>
lvTypeSysdate = lvRecord<ECB.TYPE.SYSDATE>
```

**Never use bare field names without prefix:**
```
EQU CURRENCY TO 1          ;* WRONG — missing ECB. prefix
EQU TYPE.SYSDATE TO 2      ;* WRONG — missing ECB. prefix
```

**Common prefix examples:**
| Application | Prefix | Example field |
|-------------|--------|---------------|
| `EB.CONTRACT.BALANCES` | `ECB.` | `ECB.TYPE.SYSDATE` |
| `ACCOUNT` | `AC.` | `AC.CURRENCY` |
| `CUSTOMER` | `CUS.` | `CUS.SHORT.NAME` |
| `FUNDS.TRANSFER` | `FT.` | `FT.DEBIT.AMOUNT` |
