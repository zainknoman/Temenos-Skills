...
| [references/table-schema/&lt;APP&gt;.md](references/table-schema/) | **MANDATORY before code gen** — one file per T24 application (4,596 files): field name + file position + Java alias, ground-truth from `INSERTS/I_F.<APP>` in the JARs |

**Loading guide:**
- Code generation: `class-index.md` + relevant product file + hooks files + **`table-schema/<APP>.md` for every app the code touches (see gate below)**
...
## Field Verification Gate (MANDATORY — never skip)

**Never invent a T24 field name.** Before emitting any code that reads or writes a T24 field:

1. Load `references/table-schema/<APP>.md` for every target application.
2. List every field the code will read or write.
3. Verify each field name mechanically — don't eyeball the markdown table:
   ```
   python pipeline/validate_fields.py --app ACCOUNT --fields "AC.CUSTOMER,AC.CATEGORY"
   ```
   Exit 0 = every field verified (prints position/alias/type/mandatory for each). Exit 1 = at least one field not found, with the exact missing names listed. If the `t24-knowledge` MCP server is registered (see `CLAUDE.md` P4), `lookup_fields(app)` is the same data as a structured tool call instead of a Bash invocation.
4. **If a field is missing: HALT. Ask the developer for the correct field name. Never guess.**
5. State which fields you used and confirm they're verified, e.g. "Using fields: `AC.CUSTOMER`, `AC.CATEGORY` — both verified in `table-schema/ACCOUNT.md`" — before the code block.

`table-schema/*.md` gives field name + position, plus type/mandatory/description for 59% of fields (backfilled from JavaDoc HTML — see `CLAUDE.md` Layer A). A blank type/mandatory cell means "not captured," never treat it as "optional." MV/SV status is **not** in this file at all — it comes only from the JAR-structure rule below.

If `table-schema/<APP>.md` doesn't exist for the target application, regenerate it rather than guessing field names:
```
...
| RecordLifecycle, ServiceLifecycle, ActivityLifecycle, Enquiry, L3 Java, hook, validateRecord, checkId, autoField, com.temenos | **REQUIRED SUB-SKILL:** `temenos-l3` |
...
| VVR, VIR, VAR, VCRR, NoFile, Infobasic, SUBROUTINE, GOSUB | **REQUIRED SUB-SKILL:** `temenos-infobasic` |
| jBC, .component, .b file, metamodelVersion, $PACKAGE | **REQUIRED SUB-SKILL:** `temenos-jbc` |
