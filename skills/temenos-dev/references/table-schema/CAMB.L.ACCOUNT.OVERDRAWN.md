# CAMB.L.ACCOUNT.OVERDRAWN — Table Schema

> Source: `INSERTS/I_F.CAMB.L.ACCOUNT.OVERDRAWN` in `CACSIT_CoverdraftSweep.jar` (positions/aliases via `pipeline/insert_parse.py`); type/mandatory/description via `pipeline/html_parse.py` from JavaDoc HTML.
